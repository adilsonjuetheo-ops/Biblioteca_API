...
router.get('/verificar/:usuarioId', async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`SELECT s.*, l.titulo as livro_titulo
       FROM suspensoes s
       LEFT JOIN emprestimos e ON s.emprestimo_id = e.id
       LEFT JOIN livros l ON e.livro_id = l.id
       WHERE s.usuario_id = $1 AND s.expira_em > NOW()
       ORDER BY s.expira_em DESC LIMIT 1`, [Number(usuarioId)]);
...
router.post('/', async (req, res) => {
    try {
        ...
        // Registrar suspensão
        const result = await connection_1.pool.query(`INSERT INTO suspensoes (usuario_id, emprestimo_id, dias, motivo, expira_em)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [usuarioId, emprestimoId || null, dias, motivo || 'Devolução em atraso', expiraEm]);
        // Atualizar campo bloqueado_ate no usuário
        await connection_1.pool.query(`UPDATE usuarios SET bloqueado_ate = $1 WHERE id = $2`, [expiraEm, usuarioId]);
        // Criar aviso automático para o aluno
        await connection_1.pool.query(`INSERT INTO comunicados (titulo, mensagem, destinatario, autor, criado_em)
...
router.delete('/:id', async (req, res) => {
    try {
        await connection_1.pool.query(`DELETE FROM suspensoes WHERE id = $1`, [Number(req.params.id)]);
