...
router.get('/resumo', async (_req, res) => {
    try {
        const [totalLivros, livrosDisponiveis, emprestimosAtivos, emprestimosAtrasados, emprestimosDevolvidos, ultimosEmprestimos, livrosMaisEmprestados,] = await Promise.all([
            connection_1.pool.query('SELECT COUNT(*)::int AS total FROM livros'),
            connection_1.pool.query('SELECT COUNT(*)::int AS total FROM livros WHERE COALESCE(disponiveis, 0) > 0'),
            connection_1.pool.query("SELECT COUNT(*)::int AS total FROM emprestimos WHERE status IN ('reservado', 'retirado')"),
            connection_1.pool.query("SELECT COUNT(*)::int AS total FROM emprestimos WHERE status = 'retirado' AND data_devolucao < NOW()"),
            connection_1.pool.query("SELECT COUNT(*)::int AS total FROM emprestimos WHERE status = 'devolvido'"),
